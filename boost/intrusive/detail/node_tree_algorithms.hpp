...
#ifndef BOOST_INTRUSIVE_NODE_TREE_ALGORITHMS_HPP
#define BOOST_INTRUSIVE_NODE_TREE_ALGORITHMS_HPP
...
//! node_tree_algorithms is configured with a NodeTraits class, which encapsulates the
...
class node_tree_algorithms
{
   ...
   static node_ptr unlink_leftmost_without_rebalance(const node_ptr & header)
   {
      ...
      if (leftmost_right){
         ...
         NodeTraits::set_left(header, node_tree_algorithms::minimum(leftmost_right));
   ...
   static void erase(const node_ptr & header, const node_ptr & z, F z_and_successor_fixup, data_for_rebalance &info)
   {
      ...
      if(!z_left){
         ...
         // find z's successor
         y = node_tree_algorithms::minimum (z_right);
      ...
      if(y != z){
         ...
         if(y != NodeTraits::get_right(z)){
            ...
            x_parent = y;
         node_tree_algorithms::replace_own (z, y, header);
         ...
         if(x)
            NodeTraits::set_parent(x, x_parent);
         node_tree_algorithms::replace_own (z, x, header);
         if(NodeTraits::get_left(header) == z){
            NodeTraits::set_left(header, !NodeTraits::get_right(z) ?        // z->get_left() must be null also
               ...
               node_tree_algorithms::minimum (x));
         }
         if(NodeTraits::get_right(header) == z){
            NodeTraits::set_right(header, !NodeTraits::get_left(z) ?        // z->get_right() must be null also
                              ...
                              node_tree_algorithms::maximum(x));
         }
      }
   ...
   // everything ater this point is all broken if annotations are used
   // XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX XXX
...
#endif //BOOST_INTRUSIVE_NODE_TREE_ALGORITHMS_HPP
